...
`getKnowledgeBaseTool` 和 `getKnowledgeFileTool`；知识库向量检索开关开启时再注册
`searchKnowledgeBaseTool`。模型应先通过 `listKnowledgeBasesTool` 发现当前用户可以读取的
知识库，再使用返回的 ID 查看单个知识库、文件状态或检索内容。模型产生 tool call 后由 FastAPI 服务端
执行，工具内部继续复用 workspace 和知识库授权过滤；客户端不能直接伪造工具结果。

## 安全防护

FastAPI 默认对 `/api/v1/*` 开启限流：普通接口默认每分钟 120 次，聊天每分钟 20 次，文件
接口每分钟 30 次。配置 `REDIS_URL` 和 `RATE_LIMIT_REDIS_ENABLED=true` 后，限流计数使用
Redis fixed-window counter，可在多个 FastAPI 实例之间共享；Redis 暂时不可用时自动回退到
进程内限流，避免 Redis 故障阻断业务。健康检查和 OpenAPI 文档不计入限流，生产环境仍需在
反向代理层配置可信客户端 IP。
...
解析任务使用进程内后台任务；如果服务重启或任务超过 `KNOWLEDGE_PROCESSING_STALE_SECONDS` 没有完成，
文件列表会将其标记为 `failed`，用户可以重新开始解析，避免永久停留在 `processing`。
...
PostgreSQL/Redis 集成测试不会读取普通的 `POSTGRES_URL` 或 `REDIS_URL`，必须显式提供
测试地址，避免误连共享数据库：
...
如果要验证真实 Embedding provider，再显式提供 provider 地址和专用 API Key：
