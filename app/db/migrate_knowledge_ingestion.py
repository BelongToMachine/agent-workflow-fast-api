...
MIGRATION_PATH = (
    Path(__file__).resolve().parents[2] / "migrations" / "0002_knowledge_ingestion.sql"
)
    ...
    async with get_db_connection() as connection:
        async with connection.begin():
            ...
            if bool(result.scalar()):
                print("KnowledgeFile is already present; nothing to do.")
                return 0

            if not apply:
                print(
                    "KnowledgeFile is not present. "
                    "Run with --apply to execute the migration."
                )
                return 2

            await connection.exec_driver_sql(MIGRATION_PATH.read_text(encoding="utf-8"))
            print("Knowledge ingestion migration applied successfully.")
            return 0


def main() -> None:
    parser = argparse.ArgumentParser(description="Manage knowledge ingestion tables.")
    parser.add_argument(
        "--apply",
        action="store_true",
