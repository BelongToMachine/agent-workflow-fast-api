...
| POST | `/api/v1/knowledge-bases/{knowledge_base_id}/files` | knowledge-base `manage` + `KNOWLEDGE_INGESTION_ENABLED`；自动模式还要求 `KNOWLEDGE_EMBEDDINGS_ENABLED` | multipart `file`；query：`workspace_id`、`automation?`（默认 `false`） | `202 {file:KnowledgeFileSummary}`；始终校验 PDF/XLSX/PPTX magic bytes。默认只保存文件并保持 `pending`；`automation=true` 时后台依次解析并保存 `ParsedDocument`、生成 `KnowledgeChunk`、为全部片段生成 embedding，文件状态更新为 `ready`/`failed`。自动模式不执行 AI 分析 |
...
| GET | `/api/v1/knowledge-bases/{knowledge_base_id}/files/{file_id}/chunks` | knowledge-base `manage` + `KNOWLEDGE_INGESTION_ENABLED` | query：`workspace_id`、`offset?`、`limit?` | `KnowledgeChunkListResponse`，包含每个 chunk 的 `isEmbedded` 和 `embeddingModel` |
| POST | `/api/v1/knowledge-bases/{knowledge_base_id}/files/{file_id}/chunks/embeddings/all` | knowledge-base `manage` + `KNOWLEDGE_INGESTION_ENABLED` + `KNOWLEDGE_EMBEDDINGS_ENABLED` | query：`workspace_id` | `KnowledgeChunkEmbeddingResponse`；为该文件所有缺少当前模型 embedding 的 chunk 生成向量 |
